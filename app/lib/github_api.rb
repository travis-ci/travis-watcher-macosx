class GitHubAPI
  ...
  def createAuthorization(username, password, &block)
    ...
    @client.postPath("/authorizations", parameters: { "scopes" => [ "user:email", "public_repo" ], "note" => "Temporary token to identify on travis-ci.org" }, success:proc { |operation, replyData|
  ...
  def deleteAuthorization(id)
    @client.deletePath("/authorizations/#{id}", parameters:nil, success:proc { |operation, reply|
